...
int *collision(const char *filenameT,const char *filenameS){
    ...
    for(int i=0;i<numS;i++){ //the number of sphere
        ...
        for(int j=0;j<numT;j++){ //the number of triangle
            ...
            for(int i=0;i<3;i++){
                for(int j=0;j<3;j++){
                    double v1[3]={vectc[i*3+0],vectc[i*3+1],vectc[i*3+2]};
                    double v2[3]={refvect[j*3+0],refvect[j*3+1],refvect[j*3+2]};
                    //v2 X v1
                    double*v2matrix=CrossMatrix(v2);
                    double*crossproduct;
                    crossproduct=(double*)malloc(3*sizeof(double));
                    for(int i=0;i<3;i++){
                        crossproduct[i]=v2matrix[i*3+0]*v1[0]+v2matrix[i*3+1]*v1[1]+v2matrix[i*3+2]*v1[2];
                    }
                    double ab=0;
                    for(int i=0;i<3;i++){
                        ab+=v2[i]*v2[i];
